...
class Host(Base):
    __tablename__ = "host"
    __table_args__ = (UniqueConstraint("email", "description", "category"),)
    id = sa.Column(sa.Integer, primary_key=True)
    email = sa.Column(sa.String, unique=True, nullable=False)
    description = sa.Column(sa.String, unique=True, nullable=False)
    category = sa.Column(sa.String, unique=True, nullable=False)
...
#June5th2023--added a table same code structure as Room--
class Hotel(Base):
    __tablename__ = "hotel"
    __table_args__ = (UniqueConstraint("email", "description", "category"),)
    id = sa.Column(sa.Integer, primary_key=True)
...
#June5th2023--added hotel_id--
class Place(Base):
    __tablename__ = "place"
    __table_args__ = (UniqueConstraint("host_id", "room_id", "hotel_id", "address", "city_id"),)
    id = sa.Column(sa.Integer, primary_key=True)
    host_id = sa.Column(sa.Integer, sa.ForeignKey(Host.id))
    room_id = sa.Column(sa.Integer, sa.ForeignKey(Room.id))
    hotel_id = sa.Column(sa.Integer, sa.ForeignKey(Hotel.id))
    ...
    room = sa.orm.relationship(
        Room,
        backref=sa.orm.backref("room"),
    )
    hotel = sa.orm.relationship(
        Hotel,
        backref=sa.orm.backref("hotel"),
    )
